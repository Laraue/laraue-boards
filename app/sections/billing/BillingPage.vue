    ...
    <template #default="{ data: page }">
      <section class="billing-page">
        <div class="page-heading">
          <CreditCard class="page-heading-icon" />
        ...
        <div class="usage-grid">
          ...
          <article
            v-if="page.freeTeamOrganizations"
            class="usage-card">
            <div class="usage-heading">
        ...
        <BillingTransactionsPage
          :admin="false"
          :deps="deps.transactions" />
        <BillingTransactionsPage
          v-if="canViewBilling"
          :admin="true"
          :deps="deps.adminTransactions" />
      </section>
    </template>
  </QueryState>
</template>
...
import BillingTransactionsPage from '~/sections/billing/BillingTransactionsPage.vue'
...
const { t } = useI18n({
  en: {
    ...
    description: 'See your plan and current usage.',
  ...
  ru: {
    ...
    description: 'Тариф и текущее использование ресурсов.',
...
const layoutData = useNuxtData<AppLayoutResult>(appLayoutDataKey)
const canViewBilling = computed(
  () =>
    layoutData.data.value?.status === 'success' &&
    layoutData.data.value.data.organization.canViewBilling,
)
